...
//				}
		...
		// placing out the flowers (roses and lilies)
		for (int x = startX; x < endX; x++) {
			for (int y = 0; y < Core.level.worldH; y++) {
				if (y < Core.level.worldH - 1) {
					if (Core.level.block[x][y].id == Tile.air && Core.level.block[x][y + 1].id == Tile.dirtWithGrass) {
						if (new Random().nextInt(100) < 5) {
							Core.level.block[x][y].id = Tile.roses;
						} else if (new Random().nextInt(100) < 5) {
							Core.level.block[x][y].id = Tile.lilies;
						} else if (new Random().nextInt(100) < 5) {
							Core.level.block[x][y].id = Tile.roses2;
						} else if (new Random().nextInt(100) < 5) {
							Core.level.block[x][y].id = Tile.lilies2;
						}
					}
				}
			}
